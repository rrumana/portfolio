...
function writeLocalLog(level, message, ctx = {}) {
    if (!isDev) return;
    const fn = level === "error" ? console.error : level === "warn" ? console.warn : console.debug;
    fn("[game-of-life]", message, ctx);
}

function logInfo(message, ctx = {}) {
    return writeLocalLog("info", message, ctx);
}

function logWarn(message, ctx = {}) {
    return writeLocalLog("warn", message, ctx);
}

function logError(message, ctx = {}) {
    return writeLocalLog("error", message, ctx);
}

function logDebug(message, ctx = {}) {
    return writeLocalLog("debug", message, ctx);
}
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", startGameOfLife, { once: true });
} else {
    startGameOfLife();
}
